		...
		<div id="main">
			...
			<div id="content">
				...
				<!-- Box -->
					...
					<!-- Box Head -->
					<div class="box-head">
						<h2 class="left">Chỉnh sửa loại phòng</h2>
					</div>
					<!-- End Box Head -->	

         <script>
             function huy()
             {window.location="loaiphong.php?p=1";}
         </script>
 ...
 if(isset($_POST['btnUpdate']))
        {
            ...
            move_uploaded_file($_FILES['fImg']['tmp_name'], "../../images/dichvu/$ha1");}
             $query = "update loaiphong set TenLP='$ten',ThongTin='$tt', DonGia='$gia'";
             if($ha!="")
                    {
                        $xoaanhcu="select HinhAnh from loaiphong where MaLP=$id";
                        $xoaanhcu1= mysqli_query($link, $xoaanhcu);
                        $xoa1= mysqli_fetch_array($xoaanhcu1);
                        unlink("../../images/loaiphong/$xoa1[0]");
                        $query.=", HinhAnh='$ha1'";
                    }
             $query.=" where MaLP='$id'";
             ...
             {
                 header('location:loaiphong.php?sua=ok&p=1');
             }else {
...
?><table border="2" style="margin: auto; border-radius: 20px; margin-bottom: 10px; margin-top: 10px;" width="700px" cellspacing="20">
        <form action="" method="post" enctype="multipart/form-data">
          ...
          <tr><td class="tdleft1"> Tên loại phòng:</td><td><input type="text"  value="<?php echo $r[1] ?>" name="txtTen"/></td></tr>
          <tr><td class="tdleft1">Hình ảnh:</td><td><img src="../../images/loaiphong/<?php echo $r[2] ?>" style="max-width: 300px; max-height: 300px;"/>
                                                     <input type="file" name="fImg" value="<?php echo $r[2] ?>"/></td></tr>
          ...
          <tr><td></td><td style="padding-left: 110px; "> <input class="btninput" type="submit" name="btnUpdate" value=" Lưu lại "/>&nbsp;&nbsp;

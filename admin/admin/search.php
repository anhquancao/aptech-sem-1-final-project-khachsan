...
<div id="container">
	<div class="shell">
		...
		<!-- Small Nav -->
		<div class="small-nav">
			<a href="admin.php?p=1">Quản lý Amin</a>
			<span>&gt;</span>
			Danh sách Admin
		</div>
		<!-- End Small Nav -->
					...
					<div class="box-head">
						...
						<div class="right">
                                                        ...
                                                        <form action="" method="post">
                                                            <label></label><input type="text" placeholder="Nhập tên nhân viên" class="field small-field" name="txtKey" value="<?php if(isset($_REQUEST['key'])){echo $_REQUEST['key'];}?>" <?php if(isset($_REQUEST['key'])){echo"autofocus";} if(isset($_POST['btnAdd'])){echo "autofocus='false'";}?> onfocus="this.select()"/>
...
<div style="height: 440px">
    <div style="float: right; padding-right: 10px; padding-top: 5px;"><a href="dsdangky.php?p=1">Đơn đăng ký mới(<?php
                                                                        $dangky="select count(User) from dangky ";
...
$query = "select MaAD,User,Password,TenNV,NgaySinh,DienThoai,DiaChi,Email,TrangThai,Quyen from admin where Quyen!='2'  and User!= 'admin'";
if( $k !="")
{$query.= " and TenNV like '%$k%'";}
$query.=" order by MaAD desc limit $start,$sodong";
...
echo "</table>";
 if(mysqli_num_rows($result)<1)
    {
     echo "<br/><br/><br/><br/><div style='font-size: 17px; padding-left: 380px;'>Không tìm thấy admin phù hợp</div>";
    }
                ...
                for($i=1; $i<=$sotrang; $i++)
                 {
                    if($i===$p)
                        {
                            echo "<a href='admin.php?p=$i'><b>$i</b><a>";
                        }
                    else
                        {

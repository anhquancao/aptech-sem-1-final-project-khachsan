		...
		<div id="main">
			...
			<div id="content">
				...
				<!-- Box -->
					...
					<!-- Box Head -->
					<div class="box-head">
						<h2 class="left">Chỉnh sửa quảng cáo</h2>
					</div>
					<!-- End Box Head -->	

         <script>
             function huy()
             {window.location="quangcao.php?p=1";}
         </script>
 ...
 if(isset($_POST['btnUpdate']))
        {
             ...
             $ext= explode('.', $ha)[1];
             $ha1= time().".$ext";
             move_uploaded_file($_FILES['fImg']['tmp_name'], "../../images/quangcao/$ha1");}
             $query = "update quangcao set TenQC='$ten'";
                 if($ha!="")
                    {
                        $xoaanhcu="select HinhAnh from quangcao where MaQC=$id";
                        $xoaanhcu1= mysqli_query($link, $xoaanhcu);
                        $xoa1= mysqli_fetch_array($xoaanhcu1);
                        unlink("../../images/quangcao/$xoa1[0]");
                        $query.=", HinhAnh='$ha1'";
                    }
             $query.="where MaQC='$id'";
             ...
             {
                 header('location:quangcao.php?sua=ok&p=1');
             }else {
                    echo" <div class='msg msg-error' style='width:30%'>
			<p><strong>Thông tin vẫn chưa được chỉnh sửa!</strong></p>	
             </div>";}
        }
?><table border="2" style="margin: auto; border-radius: 20px; margin-bottom: 10px;" width="00px" cellspacing="20">
        <form action="" method="post" enctype="multipart/form-data">
          <tr><td class="tdleft1"> Tên quảng cáo:</td><td><input type="text"  value="<?php echo $r[1] ?>" name="txtTen"/></td></tr>
          <tr><td class="tdleft1">Hình ảnh:</td><td><img src="../../images/quangcao/<?php echo $r[3] ?>" style="max-width: 300px; max-height: 300px;"/>
                                                    <br/><input type="file" name="fImg" value="<?php echo $r[3] ?>"/></td></tr>
          <tr><td class="tdleft1">Liên kết:</td><td><textarea name="txtLK" cols="35" rows="3"><?php echo $r[2] ?></textarea></td></tr>
           
          <tr><td></td><td>  <input style="margin-left: 35px" class="btninput" type="submit" name="btnUpdate" value=" Lưu lại "/>&nbsp;&nbsp;

		...
		<div id="main">
			...
			<div id="content">
				...
				<!-- Box -->
					...
					<!-- Box Head -->
					<div class="box-head">
						<h2 class="left">Chỉnh sửa Album</h2>
					</div>
					<!-- End Box Head -->	

         <script>
             function huy()
             {window.location="album.php?p=1";}
         </script>
...
{
    $id=$_REQUEST['id'];
    $query="select * from album where MaAlbum='$id'";
 ...
 if(isset($_POST['btnUpdate']))
        ...
        move_uploaded_file($_FILES['fImg']['tmp_name'], "../../images/album/$ha1");}
             $query = "update album set Album='$ten',MoTa='$mt'";
             if($ha!="")
                    {
                        $xoaanhcu="select AnhDaiDien from album where MaAlbum=$id";
                        $xoaanhcu1= mysqli_query($link, $xoaanhcu);
                        $xoa1= mysqli_fetch_array($xoaanhcu1);
                        unlink("../../images/album/$xoa1[0]");
                        $query.=", AnhDaiDien='$ha1'";
                    }
             $query.=" where MaAlbum='$id'";
             mysqli_query($link, $query) or die(mysqli_error($link));
             
             if(mysqli_affected_rows($link)>0)
             {
                 header('location:album.php?sua=ok&p=1');
             }else {
                    echo" <div class='msg msg-error' style='width:30%'>
			<p><strong>Thông tin vẫn chưa được chỉnh sửa!</strong></p>	
             </div>";}
        }
?><table border="2" style="margin: auto; border-radius: 20px; margin-bottom: 10px; margin-top: 10px;" width="900px" cellspacing="20">
        <form action="" method="post" enctype="multipart/form-data">
          
          <tr><td class="tdleft1"> Tên DV:</td><td><input type="text"  value="<?php echo $r[1] ?>" name="txtTen"/></td></tr>
          <tr><td class="tdleft1">Hình ảnh:</td><td><img src="../../images/album/<?php echo $r[4] ?>" style="max-width: 300px; max-height: 300px;"/>
                                                    <br/><input type="file" name="fImg" value="<?php echo $r[4] ?>"/></td></tr>
          <tr><td class="tdleft1">Mô tả: </td><td><textarea name="txtMT" class="ckeditor"><?php echo $r[2] ?></textarea></td></tr>
          <tr><td></td><td style="padding-left: 230px; "> <input class="btninput" type="submit" name="btnUpdate" value=" Lưu lại "/>&nbsp;&nbsp;
